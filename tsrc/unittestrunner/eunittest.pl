...
{  
    ...
    unlink("$eunitDstDllLocation$currentDllNameLine");
		...
		else
		{
    		...
    		system("abld test build winscw udeb >> $projectdrive$projectrootname$buildResults");
    	}
    	else
    	{
		...
		chdir($projectrootname);
		system($currentExtraCommandsLine);
...
#run all tests
print("Running: $eunitExe $eunitParams");
system("$eunitExe $eunitParams");

# store results file
copy($eunitLog, "$projectdrive$projectrootname$eunitResults");
