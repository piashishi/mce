...
_LIT8( KCryptoLine4, "1 AES_CM_128_HMAC_SHA1_80 inline:F4ksi5TJD4GDF5eodkfjtoekdlskghtye833nr3u UNENCRYPTED_SRTCP");
...
void UT_CMceSecureDesStream::UT_CMceSecureDesStream_ValidateOfferByAnswerLL(  )  
    ...
    //Test case for session parameter
    
    TRAP( error, iSecureStream->ValidateOfferByAnswerL( KCryptoLine4 ) );
	if ( error == KErrNoMemory )
        {
    	User::Leave( KErrNoMemory );
        }
